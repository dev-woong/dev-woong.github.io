...
class Search {
  ...
  private static processMatches(
    ...
    while (i < matches.length) {
      ...
      if (ellipsis && item.start - offset > lastIndex) {
        resultArray.push(
          `${
            replaceHTMLEnt(str.substring(lastIndex, lastIndex + offset))
          } [...] `,
        );
        resultArray.push(
  ...
  private generateResultTitle(resultLen, time) {
    return this.resultTitleTemplate.replace("#PAGES_COUNT", resultLen).replace(
      "#TIME_SECONDS",
      time,
    );
  }
  ...
  public async getData() {
    if (!this.data) {
      ...
      for (const item of this.data) {
        item.content =
          parser.parseFromString(item.content, "text/html").body.innerText;
      }
    }
